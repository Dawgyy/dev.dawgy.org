...
export default function Home() {
  ...
  const HeroHeader = () => (
    <div className="flex flex-col md:flex-row items-center gap-10 h-full justify-center md:justify-start px-8">
      <div className="relative group shrink-0">
        ...
        <Avatar className="h-32 w-32 md:h-48 md:w-48 border border-white/5 relative z-10 shadow-xl shadow-blue-900/5 rounded-full">
          <AvatarImage
            src="/moi.jpeg"
            alt="Alex Gerard"
            className="object-cover"
          />
          <AvatarFallback className="rounded-full bg-slate-50 text-slate-400">AG</AvatarFallback>
        </Avatar>
      </div>
      <div className="space-y-6 text-center md:text-left flex-1 min-w-0">
        <div className="space-y-2">
          <h1 className="text-5xl md:text-6xl font-bold tracking-tight text-slate-800 dark:text-slate-100">
            Alex Gerard
          </h1>
        ...
        <div className="flex gap-4 justify-center md:justify-start pt-4">
            <Button variant="ghost" size="icon" className="h-12 w-12 rounded-full bg-white/5 hover:bg-white/10 transition-colors duration-500" asChild>
              <a href="https://github.com/Dawgyy" target="_blank" rel="noreferrer"><Github className="h-6 w-6 text-slate-600" /></a>
            </Button>
            <Button variant="ghost" size="icon" className="h-12 w-12 rounded-full bg-white/5 hover:bg-white/10 transition-colors duration-500" asChild>
              <a href="https://www.linkedin.com/in/alex-gerard-46b201295/" target="_blank" rel="noreferrer"><Linkedin className="h-6 w-6 text-slate-600" /></a>
            </Button>
            <Button variant="ghost" size="icon" className="h-12 w-12 rounded-full bg-white/5 hover:bg-white/10 transition-colors duration-500" asChild>
              <a href="https://x.com/dxwgyy" target="_blank" rel="noreferrer"><Twitter className="h-6 w-6 text-slate-600" /></a>
            </Button>
            <Button variant="ghost" size="icon" className="h-12 w-12 rounded-full bg-white/5 hover:bg-white/10 transition-colors duration-500" asChild>
              <a href="mailto:gerardalexpro@gmail.com"><Mail className="h-6 w-6 text-slate-600" /></a>
            </Button>
        </div>
      </div>
    </div>
    ...
    <div className="flex flex-col h-full gap-4 p-2">
      <div className="grid grid-cols-2 gap-4">
        <div className="flex flex-col gap-2">
            <span className="text-[10px] text-slate-400 uppercase tracking-widest font-semibold pl-1">Languages</span>
            <div className="flex flex-wrap gap-2">
                <Badge variant="secondary" className="text-[10px] bg-white/5 text-slate-600 border-transparent px-2.5 h-6 font-medium shadow-none hover:bg-white/10 transition-colors">French</Badge>
                <Badge variant="secondary" className="text-[10px] bg-white/5 text-slate-600 border-transparent px-2.5 h-6 font-medium shadow-none hover:bg-white/10 transition-colors">English</Badge>
            </div>
        </div>
        <div className="flex flex-col gap-2">
            <span className="text-[10px] text-slate-400 uppercase tracking-widest font-semibold pl-1">Location</span>
      ...
      <div className="flex-1 flex flex-col pt-2">
         <span className="text-[10px] text-slate-400 uppercase tracking-widest font-semibold mb-3 pl-1">Education</span>
         <div className="flex-1 space-y-4">
            {educationItems.map((item, i) => (
            <div key={i} className="group flex flex-col gap-1 pl-3 border-l-2 border-slate-100 dark:border-slate-800 hover:border-slate-300 transition-colors">
                <div className="flex justify-between items-start gap-2">
                    <span className="text-xs font-semibold leading-tight text-slate-700 dark:text-slate-200 group-hover:text-blue-500 transition-colors">{item.title}</span>
                    <span className="text-[10px] text-slate-400 font-mono shrink-0 whitespace-nowrap">{item.endDate}</span>
                </div>
                <span className="text-[10px] text-slate-500 font-medium">{item.school}</span>
            </div>
            ))}
         </div>
      </div>
  ...
  const TechStackHeader = () => (
    <div className="h-full w-full">
      <div className="flex flex-wrap gap-2 content-start">
        {[
          '.NET', '.NET 8', '.NET Core', 'Azure', 'Azure AD', 'Azure Active Directory',
          'Azure Board', 'Azure DevOps', 'Azure Functions', 'Azure Key Vault', 'Azure Portal',
          'Azure SQL Database', 'CSS', 'CSS3', 'Git', 'HTML', 'HTML5', 'Identity framework',
          'Javascript', 'MSAL React', 'Microsoft 365', 'Microsoft Excel', 'Microsoft Teams',
          'Microsoft Word', 'Postman', 'React', 'React 19', 'React Router', 'React.js',
          'SQL', 'SQL Developer', 'Swagger', 'Transact SQL', 'UML', 'Visual Studio Code'
        ].map((tech, i) => (
          <div
            key={i}
            className="bg-white/5 rounded-full px-3 py-1.5 text-[10px] font-medium text-center whitespace-nowrap text-slate-600 dark:text-slate-300 hover:text-blue-500 hover:bg-white/10 transition-all cursor-default"
          >
            {tech}
          </div>
        ))}
  ...
  const BlogHeader = () => (
    <div className="flex flex-col h-full justify-center">
      <div className="space-y-3 flex-1 overflow-hidden p-1">
        {blogPosts.slice(0, 3).map((post) => (
          <div
            key={post.slug}
            className="group flex items-start justify-between gap-4 p-4 rounded-2xl hover:bg-white/5 transition-colors duration-300 cursor-pointer"
          >
            ...
            <div className="h-8 w-8 rounded-full bg-white/5 flex items-center justify-center transition-colors shrink-0">
              <ExternalLink className="h-3.5 w-3.5 opacity-40 group-hover:opacity-100 text-blue-500" />
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div
      className="pt-[8em] pb-8 px-4 md:px-6 relative w-full h-screen overflow-y-auto scrollbar-hide"
    >
            ...
            description={
              <div className="flex flex-col gap-2 pt-1">
                <span className="text-xs text-muted-foreground line-clamp-2">
                  Enhance your server with powerful automation.
                </span>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <Badge variant="outline" className="text-[10px] border-white/10 h-5 px-1.5">
                    25k+ Users
                  </Badge>
                </div>
              </div>
